var randDB = new listDB("randomThings"),
	repliesDB = new jsonDB("actback/replies");
...
listen({
	...
	callback: function (input) {
		var randThings = randDB.getEntire(),
			randReplies = repliesDB.getEntire(),
			randReply,
		...
		setTimeout(function () {
			if (!randReplies[verb] && !randReplies.alts[verb]) {
				...
				irc[method](input.context, Funcs.supplant(randReply, suppVars));
			} else {
