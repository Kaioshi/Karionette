...
var adminDB = new listDB('admins');

function isAdmin(username) {
	return adminDB.hasValue(username, true);
}
...
listen_admin({
	...
	callback: function (input) {
		...
		if (args[0]) {
			switch (args[0]) {
			case "add":
				adminDB.store(args[1]);
				irc.say(input.context, "Added :)");
				break;
			case "remove":
				adminDB.remove(args[1], true);
				irc.say(input.context, "Removed :)");
				break;
			case "list":
				irc.say(input.context, "Admins: " + adminDB.getEntire().join(", "));
				break;
			default:
