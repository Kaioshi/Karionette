var aliasDB = new jsonDB("alias/alias"),
	varDB = new jsonDB("alias/vars");
		...
		if (args[0]) {
			...
			case "append":
				if (args[1] && varString) {
					...
					if (gotten) {
						varDB.store(varName, gotten + " " + varString);
					} else {
						varDB.store(varName, varString);
					}
			...
			case "seppend":
				...
				if (args[1] && args[2] && varString) {
					...
					gotten = varDB.getOne(varName);
					if (gotten) {
						varDB.store(varName, gotten + (args[2] === "," ? ", "
							: " " + args[2] + " ") + varString.trim());
					} else {
						varDB.store(varName, varString.trim());
					}
					irc.say(input.context, "Added o7");
				} else {
			...
			case "seprem":
				...
				if (args[1] && args[2] && varString) {
					...
					if (gotten) {
						if (gotten === varString) {
							varDB.remove(varName);
							irc.say(input.context, "Removed o7");
						} else {
							varDB.store(varName, gotten.split((args[2] === "," ? ", " : " " + args[2] + " "))
