...
function FileDB(options) {
	...
	this.pathExists = fs.existsSync(options.filename);

	var Q = options.queue || false,
		noCreate = options.noCreate || false,
		init = pathCheck(options.filename, this.pathExists, noCreate, this._defaultData);
	...
	if (Q) {
		this.queue.subscribe(options.filename);
	}
}
...
FileDB.prototype._writeFile = function writeFile(data) {
	if (this.queue.isSubscribed(this._filename)) {
		...
		fs.writeFile(this._filename, data, function (err) {
			if (err) {
