...
var FileDB = (function () {
	...
	return function ConstructFileDB(options) {
		...
		if (Q) {
			FileDB.queue.subscribe(this._filename);
		}
...
FileDB.queue = {
	...
	saveCache: function () {
		var eachKey, cKeys;
		cKeys = Object.keys(this.cache);

		if (cKeys.length > 0) {

			eachKey = function eachCacheKey(element) {
				try {
					fs.writeFileSync(element, this.cache[element]);
					logger.info("Data saved to " + element);
				} catch (err) {
					logger.error("There was a problem writing to : " + element + " " + err, err);
				}
...
FileDB.prototype._readFile = function readFile() {
	var data = FileDB.queue.cache[this._filename] || fs.readFileSync(this._filename, 'utf8');
	return data;
};
...
Json.prototype.saveAll = function saveAllJson(obj) {
	this._writeFile(JSON.stringify(obj, null, 3));
};
...
List.prototype.getAll = function getAllList() {
	return this._parseArray(this._readFile());
};
