...
var queue = {
	...
	saveCache: function () {
		var cKeys = Object.keys(this.cache),
			that = this;
		if (cKeys.length > 0) {
			log2("info", "Queue Write...");
			cKeys.forEach(function (element) {
				fs.writeFile(element, that.cache[element], function (err) {
					if (err) {
						log2("error", "There was a problem writing to: " + element + " " + err);
					} else {
						log2("info", "Data saved to " + element);
					}
				});
	...
	saveOneCache: function (filename) {
		fs.writeFile(filename, this.cache[filename], function (err) {
			if (err) {
				log2("error", "There was a problem writing to: " + filename + " " + err);
			} else {
				log2("info", "Data saved to " + filename);
			}
		});
	},
	subscribe: function (path) {
		if (this.queuees.length === 0 && !this.on) {
			this.fullStart();
			log2("info", "Write Queue initiating...");
		}
...
function pathCheck(filename, exists, noCreate, data) {
	...
	if (!exists) {
		if (!noCreate) {
			mkdirPath(filename, function (err) {
				if (!err) {
					fs.writeFile(filename, data, function (err) {
						if (err) {
							log2("error", "Couldn't create: " + filename);
						} else {
							log2("info", "Created file: " + filename);
						}
					});
				} else {
					log2("error", err);
			...
			log2("warn", "noCreate flag set to TRUE, so did not create: " + filename);
			return false;
		}
	}
	return true;
