...
var queue = {
		...
		if (cKeys.length > 0) {
			...
			cKeys.forEach(function (element) {
				fs.writeFile(element, that.cache[element], function (err) {
					if (err) {
						console.log("[Error] There was a problem writing to:", element, err);
					} else {
	...
	saveOneCache: function (filename) {
		fs.writeFile(filename, this.cache[filename], function (err) {
			if (err) {
				console.log("[Error] There was a problem writing to:", filename, err);
			} else {
...
function pathCheck(filename, exists, noCreate, data) {
	...
	if (!exists) {
		if (!noCreate) {
			...
			console.log("[WARNING] noCreate flag set to TRUE, so did not create:", filename);
			return false;
		}
	}
	return true;
