			...
			mkdirPath(filename, function (err) {
				if (!err) {
					try {
						...
						logger.error("Couldn't create "+filename+": "+err);
					}
				} else {
					logger.error(err);
					return false;
...
exports.Json = function (options) {
	var Q = options.queue || false,
		...
		pathExists = (fs.existsSync(filename) && fs.lstatSync(filename).size > 0),
	...
	return {
		...
		getAll: function () {
			if (!fs.existsSync(filename) || fs.lstatSync(filename).size === 0) return "{}";
...
exports.List = function (options) {
	...
	return {
		size: function () {
			return fs.lstatSync(filename).size;
		},
		getAll: function () {
		...
		getOne: function (data, ignoreCase) {
			var has = parseArray(readFile(filename)).some(function (element) {
				if (ignoreCase) return (element.toUpperCase() === data.toUpperCase());
				return (element === data);
			});
		...
		removeOne: function (data, ignoreCase) {
			var all = this.getAll().filter(function (element) {
				if (ignoreCase) return (element.toUpperCase() !== data.toUpperCase());
				return (element !== data);
			});
			this.saveAll(all);
