...
function overwriteDB(db, obj) {
	...
	if (!newkeys.length) {
		...
		logger.info("Would be wiping "+dbname+" and resetting keymap - saveAll was passed an empty object.");
	...
	for (; i < l; i++) {
		...
		try {
			...
			logger.info("Couldn't save "+dbname+" object "+newkeys[i]+" : "+newkeymap[newkeys[i]]);
		}
	}
		...
		try {
			...
			if (old) {
				if (fs.existsSync(old)) {
					logger.debug("Converting flat db to fragmented db ...");
...
function getFullDB(db) {
	...
	var key, dbname;
	if (Object.keys(keymap[db]).length === Object.keys(objCache[db])) {
