...
web.bing = function (term, maxResults) {
	var uri = "https://api.datamarket.azure.com/Bing/Search/Web?$format=json&Query='"+
		term+"'&Market='en-us'&$top="+(maxResults ? maxResults : 1),
		auth = new Buffer(irc_config.api.bing+":"+irc_config.api.bing).toString('base64');
	return web.json(uri, { headers: [ "Authorization: Basic "+auth ] }).then(function (b) {
		...
		if (!b.d.results.length)
			...
			b.d.results.forEach(function (item) {
				ret.push({
					title: lib.singleSpace(b.d.results[item].Title),
					url: b.d.results[item].Url,
					content: b.d.results[item].Description
				});
			});
			return ret;
		}
