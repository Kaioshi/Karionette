...
if (process.platform === "linux") {
	...
	web.get = function (uri, callback, length) {
		var opts = { timeout: 15000 },
			args = [ "-sSLA \"Mozilla/5.0 (Windows NT 6.3; WOW64; rv:39.0) Gecko/20100101 Firefox/39.0\"", encodeURI(uri) ];
		if (length > 0)
...
web.json = function (uri, ignoreErrors) {
	return web.fetch(uri).then(JSON.parse).catch(function (error) {
		if (!ignoreErrors)
			logger.debug("Couldn't JSON.parse "+uri);
		else
