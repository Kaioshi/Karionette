 ...
 *  @exports FileDB()
 *  @brief Generic FileDB to extend from. Any class extendind this needs to implement methods
 *         _parse() and _stringify() to function properly.
 */
var fs = require('fs');
 ...
 *  
 *  @param data  STRING|OBJECT Data to write
 *  @return void
 *  
...
FileDB.prototype.size = function fileSize() {
	...
	if (FileDB.queue.isSubscribed(this._filename)) {
		...
		if (typeof data === "string") {
			...
			size = this._stringify(data).length;
		}
	} else {
		size = fs.lstatSync(this._filename).size;
		size = ((size === 2 && this.type ===  "Json") ? 0 : size);
	}
	return size;
};
