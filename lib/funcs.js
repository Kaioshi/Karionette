...
var fs = require('fs'),
	...
	inteSupp = function (str, obj) { // this matches on the entire '{word}' rather than 'word'
...
global.lib = {
	...
	checkArgs: function (context, command, args, length) { // doesn't cover all cases, just the basic first check
		if (args === undefined || args.length === 0) { // length = minimum expected args
			mari.say(context, caveman.cmdHelp(command, "syntax"));
			return false;
		}
		if (length !== undefined && args.length < length) {
			mari.say(context, caveman.cmdHelp(command, "syntax"));
			return false;
		}
		return true;
	...
	fs: {
		makePath: function (p) {
			var i = 0, path = p.split("/"), l = p.length, tmp = "";
			for (; i < l; i++) {
				if (!path[i].length)
					break;
				tmp += path[i]+"/";
				if (!fs.existsSync(tmp)) {
					try {
						...
						logger.error("Couldn't create "+tmp+" directory: "+err, err);
						return false;
					}
				}
			}
