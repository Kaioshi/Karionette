"use strict";
// General Helpers
var memuseLast = process.memoryUsage().rss,
...
global.lib = {
	...
	memReport: function() {
		...
		if (memuse !== memuseLast) {
			report = commaNum((memuse/1024).toString());
			diff = ((memuse-memuseLast)/1024);
			if (diff > 0) {
				diff = " [+"+space(commaNum(diff.toString()), 5)+" KiB]";
			} else {
				diff = " [-"+space(commaNum(diff.toString().slice(1)), 5)+" KiB]";
			}
