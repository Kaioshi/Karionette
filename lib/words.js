...
words = {
	...
	load: function (type) {
		words[type].list = [];
		words[type].list = fs.readFileSync("data/actback/"+type+"s.txt").toString().split("\n");
	},
	save: function (type) {
		words[type].list = removeEmpty(words[type].list);
		fs.writeFileSync("data/actback/"+type+"s.txt", words[type].list.join("\n"));
	},
	...
	noun: {
		...
		random: function() {
			return randSelect(words.noun.list);
		}
	},
	adjective: {
		add: function (word) {
		...
		random: function () {
			return randSelect(words.adjective.list);
		}
	},
	adverb: {
		add: function (word) {
