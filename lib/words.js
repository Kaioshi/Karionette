...
words = {
		...
		result.primaries.forEach(function (primary) {
			if (type === "adverb") {
				if (primary.terms[0].labels[0].text === "Adverb") {
					fs.appendFileSync("data/actback/adverbs.txt", "\n"+word);
	...
	lookup: function (type, word) { // looks up words via google define and adds them if they meet the criteria
		...
		if (type === "adverb" && words.adverb.get(word, true)) {
			logger.debug("Didn't add "+word+" to the adverb list");
			return;
		} else if (type === "verb" && words.verb.get(word, true)) {
