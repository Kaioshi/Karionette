"use strict";
var fs = require('fs'), web = require('./web.js');
...
global.words = {
	lookup: function (type, word) {
		...
		case "noun":
			...
			uri = "http://api.wordnik.com:80/v4/word.json/"+word+
				"/relatedWords?useCanonical=true&relationshipTypes=noun-form&limitPerRelationshipType=10&api_key="+irc_config.api.wordnik;
			web.get(uri, function (error, response, body) {
				body = JSON.parse(body);
				globals.lastWord = body;
		...
		case "verb":
			...
			uri = "http://api.wordnik.com:80/v4/word.json/"+word+
				"/relatedWords?useCanonical=true&relationshipTypes=verb-form&limitPerRelationshipType=10&api_key="+irc_config.api.wordnik;
			web.get(uri, function (error, response, body) {
				try {
					body = JSON.parse(body);
				} catch (e) {
				...
				if (body.length > 0) {
					...
					if (verb.s && verb.ed && verb.ing) {
						if (verb.base) {
							...
							uri = "http://api.wordnik.com:80/v4/word.json/"+word+
								"/relatedWords?useCanonical=false&relationshipTypes=verb-stem&limitPerRelationshipType=10&api_key="+irc_config.api.wordnik;
		...
		default:
			uri = "http://api.wordnik.com:80/v4/word.json/"+word+
				"/definitions?limit=1&includeRelated=false&sourceDictionaries=all&useCanonical=false&includeTags=false&api_key="+irc_config.api.wordnik;
			web.get(uri, function (error, response, body) {
				try {
					body = JSON.parse(body);
				} catch (e) {
	...
	noun: {
		add: function (word) {
			...
			// make sure the noun isn't there already
			var noun = word.split(" ");
			if (words.noun.get(noun[0]) || words.noun.get(noun[1]))
				return "I'm already familiar with " + noun[0];
			words.noun.list.push(word);
			words.save("noun");
			return "Added o7";
		},
		remove: function (word) {
			//return words.remove("noun", word);
			var noun, i = 0, l = words.noun.list.length;
			word = word.split(" ");
			for (; i < l; i++) {
				if (words.noun.list[i].indexOf(word[0]) > -1) {
					noun = words.noun.list[i].split(" ");
					if (noun[0] === word[0]) {
						words.noun.list.splice(i, 1);
						words.save("noun");
						return "Removed o7";
					}
				}
			}
			return "I'm not familiar with " + word[0];
		},
		get: function (word) {
			...
			word = word.toLowerCase();
			for (; i < l; i++) {
				if (words.noun.list[i].indexOf(word) > -1) {
					noun = words.noun.list[i].split(" ");
					if (word === noun[0] || word === noun[1]) {
		...
		random: function() {
			var noun = lib.randSelect(words.noun.list).split(" ");
			return { base: noun[0], s: noun[1] };
		}
	},
