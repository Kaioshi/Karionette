...
timers = {
	...
	Add: function (interval, func) {
		...
		if (this.timers.length > 0) {
			for (i = 0; i < this.timers.length; i++) {
				if (this.timers[i].func === funcStr && this.timers[i].args === args.join(" ")) {
	...
	Remove: function (interval, func) {
		...
		for (i = 0; i < this.timers.length; i++) {
			if (this.timers[i].freq === interval) {
				if (this.timers[i].func === func && this.timers[i].args === args) {
