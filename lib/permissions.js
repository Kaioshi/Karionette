...
permissions = {
	...
	Example: "Example: "+config.command_prefix+"permissions deny add variable anime_list mitch*!*@*",
	...
	Allow: {
		...
		Example: "Example: "+config.command_prefix+
	...
	Admin: {
		...
		Secret: function (from, secret, mask) {
			...
			if (from && from.match(/[^ ]+![^ ]+@[^ ]+/) && secret === irc_config.secret) {
				...
				return "[Help] Syntax: "+config.command_prefix+"secret <secret> [mask]";
			}
		}
	}
};
