	...
	emitEvent: function (input) {
		...
		if (input[0][0] === ":") {
			...
			if (type === "PRIVMSG") {
				...
				if (params.command) {
					if (caveman.commands[params.command] === undefined) {
						params.raw = transformAlias(params.raw, params.command);
		...
		if (input[0].indexOf("!") > -1) {
			...
			params.data = params.raw.slice(1);
			...
			case "PRIVMSG":
				...
				if (params.command) {
					if (caveman.commands
						...
						&& !permissions.isAdmin(params.nick+"!"+params.address)) {
		...
		if (caveman.eventRegexes[type]) {
			...
			//logger.debug(i+" regexes were run, taking "+(new Date().valueOf()-before)+"ms.");
...
function getCommand(input) {
	input[3] = input[3].slice(1).toLowerCase();
	if (input[3][0] === irc_config.command_prefix) return input[3].slice(1);
	if (input[4]) {
		input[3] = input[3].replace(/\,|\:|\-/, "");
		input[4] = input[4].toLowerCase();
		if (input[3] === irc_config.nick
			|| irc_config.nickname.some(function (entry) { return (entry.toLowerCase() === input[3]); })) {
			return input[4];
		}
	}
}
...
function transformAlias(line, command) {
	var i, toTransform, aliasMatch, alias,
		aliasVars, context, nick, data,
		aliasKeys = aliasDB.getKeys();
	for (i = 0; i < aliasKeys.length; i += 1) {
		aliasMatch = regexFactory.startsWith(aliasKeys[i]).exec(line);
		if (aliasMatch) {
