			...
			if (type === "PRIVMSG") {
				...
				if (params.command) {
					if (params.command[0] === "alias") {
						...
						params.aliasArgs = input.slice(4);
					...
					if (caveman.commands && caveman.commands[params.command] && caveman.commands[params.command].admin && !userLogin.isAdmin(params.user)) {
						...
						if (permission) {
							if (aliasSyntax) // no command should happen if an alias syntax was wrong
								mari.say(params.context, aliasSyntax, false);
							else if (cmdArglen) // not enough args supplied
								mari.say(params.context, cmdArglen, false);
							else
...
function transformAlias(line, command) {
	...
	if (aliasMatch[1].indexOf("|") > -1 && !aliasMatch[1].match(/\{\((.*\|?)\)\}|\{\[(.*\|?)\]\}/)) {
...
function replaceVars(match, context, from, line) {
	...
	if (!argsDone) {
		if (line.indexOf("{args") > -1) {
			...
			if (line.indexOf("{args1*}") > -1) line = line.replace(/\{args1\*\}/g, (args && args.length > 0 ? match[1] : ""));
	...
	if (varParseLimit > 0 && line.match(/\{[^\[\(\|\)\] ]+\}/)) line = replaceVars(match, context, from, line);
	else {
		varParseLimit = 3;
		argsDone = false;
	}
