...
function ignored(input) {
	var channel;
	logger.debug("Checking ignores...");
	if (input[0].indexOf("!") > -1) {
		if (ignoreDB.getOne(input[0].slice(1, input[0].indexOf("!")))) {
			return true;
		}
	}
	channel = (input[2][0] === ":" ? input[2].slice(1) : input[2]);
	if (channel[0] === "#") {
		if (ignoreDB.getOne(channel)) return true;
	}
	logger.debug("None!");
}
...
caveman = {
	emitEvent: function (input) {
		...
		input = input.split(" ");
		if (input[0][0] === ":") {
			if (ignoreDB.size() > 0 && ignored(input)) {
				return; // ignored
			}
			type = input[1];
			if (type === "PRIVMSG") {
