...
caveman = {
	...
	commandListen: function (command) {
		...
		if (!(command.command && command.help && command.callback)) {
			logger.error("Incorrecet caveman.commandListen format - need at least a command, help and callback.");
			return;
		}
		funcStr = command.callback.toString();
	...
	eventListen: function (event) {
		...
		if (!(event.event || event.command) || !(event.handle && event.callback)) {
			logger.error("Incorrect caveman.eventListen format - need at least an event (or command), handle and a callback.");
			return;
		}
		...
		if (caveman.events[event.handle]) {
			logger.debug("Removed stale "+event.handle+" event");
			events = lib.events.listeners(trigger);
			for (i = 0; i < events.length; i++) {
				if (events[i].toString() === caveman.events[event.handle].func) {
					logger.debug("Removed event listener for stale "+event.handle);
