...
function transformAlias(line, command) {
	...
	//if (toTransform.indexOf("℅") > -1) { // magic!
	//	toTransform = toTransform.replace(/℅/g, "|");
	//}
	//if (aliasMatch[1]) {
...
function replaceVars(match, context, from, line) {
	...
	while ((reg = /(\{[^\{\|\(\)\[\]\} ]+\})/.exec(tmp))) {
		...
		switch (reg[1]) {
			case "{args*}":
				line = line.replace(reg[1], match[1]);
				break;
			case "{args1}":
				line = line.replace(reg[1], (args && args.length > 0 ? args[0] : ""));
				break;
			case "{args2}":
				line = line.replace(reg[1], (args && args.length > 1 ? args[1] : ""));
				break;
			case "{args3}":
				line = line.replace(reg[1], (args && args.length > 2 ? args[2] : ""));
				break;
			case "{args4}":
				line = line.replace(reg[1], (args && args.length > 3 ? args[3] : ""));
				break;
			case "{args1*}":
				line = line.replace(reg[1], (args && args.length > 0 ? args.slice(1).join(" ") : ""));
				break;
			case "{args2*}":
				line = line.replace(reg[1], (args && args.length > 1 ? args.slice(2).join(" ") : ""));
				break;
			case "{args3*}":
				line = line.replace(reg[1], (args && args.length > 2 ? args.slice(3).join(" ") : ""));
				break;
			case "{args4*}":
				line = line.replace(reg[1], (args && args.length > 3 ? args.slice(4).join(" ") : ""));
				break;
			default:
...
/*function makeVars(match, context, from, data) {
	var reg, variable, nicks, newMatch, args, i, tmp,
		ret = {};
	while ((reg = /(\{[^\|\(\)\[\]\} ]+\})/.exec(data))) {
		switch (reg[1]) {
			case "{me}":
				ret[reg[1]] = magicInputFondler(irc_config.nick);
				break;
			case "{from}":
				ret[reg[1]] = magicInputFondler(from);
				break;
			case "{whippingBoy}":
				ret[reg[1]] = magicInputFondler(lib.randSelect(irc_config.local_whippingboys));
				break;
			case "{channel}":
				ret[reg[1]] = magicInputFondler(context);
				break;
			case "{randThing}": ret[reg[1]] = lib.randSelect(randDB.getAll()); break;
			case "{randNick}":
				nicks = (context[0] === "#" ? ial.Active(context) : []);
				nicks = nicks.filter(function (nick) { return (nick !== from); });
				nicks = (nicks.length > 0 ? nicks : [ "someone", "The Lawd Jesus", "your dad", "Santa" ]);
				ret[reg[1]] = magicInputFondler(lib.randSelect(nicks));
				nicks = null;
				break;
			...
			default:
				...
				if (reg[1][1] === "#") {
					...
					if (tmp) {
						...
						if (tmp[1] > 100) { // minimum needs to be reasonably low so we don't loop random() for ages.
							logger.warn("Someone is trying to do a stupid random number. replacing it with a note instead.");
							ret[reg[1]] = "{#MinTooHigh-100-Max}";
							break;
						}
						tmp[2] = parseInt(tmp[2], 10);
						if (tmp[1] >= tmp[2]) {
							logger.warn("Someone is trying to do a stupid random nuymber. replacing it with a note instead.");
							ret[reg[1]] = "{#Min-Max--MinNeedsToBeLower}";
							break;
						}
						ret[reg[1]] = randRange(tmp[1], parseInt(tmp[2],10)).toString();
						break;
					}
				}
				// must be a variable name, or jibberish.
	...
	if (match[1]) {
		if (match[1].indexOf("|") > -1 && match[1].indexOf("{(") === -1 && match[1].indexOf("{[") === -1) {
			newMatch = lib.supplant(magicInputFondler(match[1]), ret); // This is Ugly. Blame Uni
		} else {
			newMatch = lib.supplant(match[1], ret); // How dare she discover bugs that needed fixing.
		}
		args = newMatch.split(" ");
		ret["{args*}"] = newMatch;
