...
module.exports = function (lib, config, logger, ial, perms, words, userLogin, aliasDB, varDB, helpDB, ignoreDB, randDB) {
	var alias = require("./alias.js")(lib, config, ial, words, varDB, randDB),
		cmEvents = {},
		cmCommands = {},
		cmCommandAliases = {},
		cmEventRegexes = {},
	...
	function checkIgnored(input, ignoreCache, ignored) {
		...
		for (i = 0; i < ignored.length; i++) {
			if (ial.maskMatch(user, ignored[i])) {
	...
	function getCommand(input, commands, commandAliases, aliasDB) {
		...
		if (command) {
			if (commands[command] || commandAliases[command])
				return [ "command", command, pos ];
			if (aliasDB.hasOne(command))
	...
	return {
		emitEvent: function emitEvent(input) {
			...
			input = input.trim();
			logger.filter(input);
			if (ignoreDB.size() > 0 && checkIgnored(input, cmIgnoreCache, cmIgnored))
				return; // ignored
			params = { raw: input };
			input = input.split(" ");
			if (input[0][0] === ":") {
				type = input[1];
				if (type === "PRIVMSG") {
					cmd = getCommand(input, cmCommands, cmCommandAliases, aliasDB);
					if (cmd)
						params.command = cmd;
					if (params.command) {
						if (params.command[0] === "alias") {
							...
							params.raw = alias.transform(params.raw, params.command[1],
									aliasDB.getOne(params.alias), params.aliasArgs,
									varDB, randDB);
			...
			case "PRIVMSG":
				...
				if (params.command) {
					...
					} else if (cmCommands[params.command].arglen !== undefined &&
								params.args !== undefined &&
								params.args.length < cmCommands[params.command].arglen) {
