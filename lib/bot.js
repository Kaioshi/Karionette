...
module.exports = function (lib, config, logger, edgar, ial, perms, words, userLogin, alias, ignore) {
	var bot = {};
	...
	bot.parse = function botParse(input) {
		...
		case "PRIVMSG":
			...
			if (params.command) {
				if (edgar.commandNeedsAdmin(params.command) && !userLogin.isAdmin(params.user)) {
