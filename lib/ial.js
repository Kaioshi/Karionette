...
global.ial = {
	...
	maskSearch: function (mask, channel) {
		...
		var chans, i, l, k, n, done, matches = [];
		if (mask) {
			mask = new RegExp(mask.replace(/\./g, "\\.").replace(/\?/g, ".").replace(/\*/g, "([^ ]+|)"), "i");
			if (channel && globals.channels[channel].users) {
				...
				chans = Object.keys(globals.channels); l = chans.length; i = 0;
				for (; i < l; i++) {
