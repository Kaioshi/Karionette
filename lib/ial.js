...
ial = {
	...
	User: function (nick, channel) {
		...
		if (globals.channels[channel].users) {
			Object.keys(globals.channels[channel].users).forEach(function (entry) {
				if (entry.toLowerCase() === nick.toLowerCase()) {
					ret = globals.channels[channel].users[entry];
				}
			});
		}
		return ret;
	...
	Active: function (channel) {
		// returns a list of people who have spoken in 10 minutes
