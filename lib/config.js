...
function parseEntry(entry) {
	...
	if (entry[1].toLowerCase() === "true") return true;
	if (entry[1].toLowerCase() === "false") return false;
	return entry[1];
}
...
function parseConf(conf) {
	var i = 0, entry, config = {}, l = conf.length;
	for (; i < l; i++) {
		if (conf[i] && conf[i][0] !== "#") {
...
global.irc_config.saveChanges = function () { // saves changes and returns how many changes were made.
	...
	for (field in irc_config) {
		if (irc_config.hasOwnProperty(field)) {
			if (field === "nick" || field === "address" || field === "saveChanges")
				continue;
			if (field === "api") {
				for (subfield in irc_config[field]) {
					if (irc_config[field].hasOwnProperty(subfield)) {
	...
	for (field in newconf) {
		if (newconf.hasOwnProperty(field)) {
