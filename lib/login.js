...
userLogin = {
	...
	Check: function (from, admin) { // returns username if they're logged in with their nick!user@host
		var i, keys;                // if admin=true, checks if they're an admin.
		
		if (userLogin.loginCache && userLogin.loginCache[from]) { // say no to loops!
			logger.debug("Fast path bitches!");
			if (admin && !userLogin.loggedIn[userLogin.loginCache[from]].admin) {
				return false;
			}
			return userLogin.loginCache[from];
		}
		logger.debug("Slow path mitches ;<");
