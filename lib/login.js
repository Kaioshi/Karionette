...
module.exports = function (lib, config, logger, FragDB, ial, eventListen) {
	var usersLoggedIn = {},
		userLoginCache = {},
	...
	restoreLoginState();
	eventListen({ handle: "loginStateValidator", event: "autojoinFinished", callback: validateLoginState });
	...
	function checkUser(opts) {
		...
		if (opts.admin)
			return user;
		if (user.password === opts.password || config.secret === opts.password)
			return user;
	}

	function restoreLoginState() { // what is going on here? ._____.
	...
	function removeFromCache(username) {
		Object.keys(userLoginCache).forEach(function (user) {
			if (userLoginCache[user] === username) {
				logger.debug("Removing duped "+username+" userhost: "+user);
				delete userLoginCache[user];
			}
		});
	}
	...
	return {
		...
		Logout: function (from) {
			delete usersLoggedIn[userLoginCache[from]];
			delete userLoginCache[from];
			saveState();
		},
		Update: function (oldUser, newUser) { // nick changes etc.
			var user = userLoginCache[oldUser];
			delete userLoginCache[oldUser];
			userLoginCache[newUser] = user;
			usersLoggedIn[user].user = newUser;
			saveState();
			console.log(userLoginCache, usersLoggedIn);
		},
		...
		isLoggedIn: function (from) {
			if (userLoginCache[from] !== undefined)
				return true;
			return false;
		},
