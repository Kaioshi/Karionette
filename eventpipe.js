 ...
 */
var JsonDB = require('./lib/db_json.js'),
...
module.exports = (function () {
	var keyCache, aliasVars,
		...
		isInAlias = false,
		maxEventFire = 10,
		aliasDB = new JsonDB("alias/alias"),
		varDB = new JsonDB("alias/vars"),
		randThings = new ListDB("randomThings").getEntire();
	...
	function makeVars(match, context, from) {
		...
		if (match[1]) {
			newMatch = Funcs.supplant(match[1], av);
			args = newMatch.split(" ");
			av["{args*}"] = newMatch;
	...
	function evaluateAlias(aliasString, aliasVars) {
		return Funcs.supplant(aliasString, aliasVars);
	}
