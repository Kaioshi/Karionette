...
const words = {
	...
	remove: function (type, word) {
		...
		if (match) {
			if (type === "noun")
				...
				words[type].list.removeOne(word);
			return "Removed. o7";
		}
	...
	noun: {
		...
		get: function (word) { // needs to match either singular or plural
			...
			if (word[word.length-1] === "s")
				...
				matches = words.noun.list.search(word+" ");
			if (matches.length) {
				let noun = lib.randSelect(matches).split(" ");
				return { base: noun[0], s: noun[1] };
			}
		},
		random: function() {
			let noun = words.noun.list.random().split(" ");
			return { base: noun[0], s: noun[1] };
		}
	},
	verb: { // this needs to be redone from scratch - too simple and too hard to verify syntax
		...
		add: function (line) {
			let lline = line.toLowerCase(), match;
		...
		get: function (word) {
			let lword = word.toLowerCase(), match;
		...
		random: function () {
			let line = words.verb.list.random().split(" ");
